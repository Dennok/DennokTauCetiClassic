...
/mob/living/carbon/monkey
		...
		if(losebreath>0) //Suffocating so do not take a breath
			...
			if(!breath)
				if(istype(loc, /obj/))
					...
					var/breath_moles = environment.cell_moles()*BREATH_PERCENTAGE
	...
	proc/handle_breath(datum/gas_mixture/breath)
		...
		var/O2_pp = (breath.oxygen/breath.cell_moles())*breath_pressure
		// Same, but for the phoron
		var/Toxins_pp = (breath.phoron/breath.cell_moles())*breath_pressure
		// And CO2, lets say a PP of more than 10 will be bad (It's a little less really, but eh, being passed out all round aint no fun)
		var/CO2_pp = (breath.carbon_dioxide/breath.cell_moles())*breath_pressure
